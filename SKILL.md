...
description: Set up, validate, troubleshoot, and repair Codex Desktop remote projects over SSH, especially from a Windows client to an always-on Windows or WSL host through Tailscale. Use for Codex SSH Connections, remote project setup, OpenSSH keys and ssh-agent, remote Codex PATH or app-server failures, unstable or reset connections, ProxyJump and host-key errors, missing remote chats, VPN placement, WSL/Windows Codex-home separation, and safe chat migration between computers. Do not use for generic SSH work unrelated to Codex.
---
