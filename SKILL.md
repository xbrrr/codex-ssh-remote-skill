...
description: Set up, diagnose, repair, and stabilize Codex Desktop SSH Remote Connections and remote projects between computers, especially Windows clients connecting to always-on WSL, Linux, or Windows hosts through Tailscale or mesh VPN. Use for Codex over SSH, Settings / Connections / SSH, computer A/B setups, OpenSSH/sshd, SSH keys, ssh-agent, BatchMode, known_hosts, Connection failed/reset/flapping, ProxyJump or UNKNOWN port 65535, PATH or codex not found, app-server startup, config.toml/service_tier mismatch, VPN placement, files visible but No chats, separate Windows/WSL Codex homes or session indexes, project folders versus chat storage, Git Hand off restrictions, projectless chat migration, duplicate/disappearing task IDs, and JSONL session verification. Use to choose a direct WSL/Linux topology, prove network/auth/shell/app-server gates, add remote folders, explain synchronization limits, and move tasks safely. Do not use for generic SSH unrelated to Codex or claim SSH creates an exact desktop mirror.
---
